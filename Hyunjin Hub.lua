local function __HyunjinHub_Init_Main__()
...
local UIConfigPath = "HyunjinHub_UI_Theme.json"
...
if ModernV2 then
    ...
    Window:CreateHomeTab({
        ...
        Changelog = {
            {
                Title = "HyunjinHub crack VD v1.5.7",
...
function KYS_CreateWindowAdornment(part)
    if not part or not part.Parent or getgenv().KYS_WindowESPObjects[part] then
        return
    end

    local color = VD.ESP_WindowColor or Color3.fromRGB(255, 255, 255)
    ...
    adorn.ZIndex         = 10
    adorn.Parent         = workspace   -- atau GetSafeChamsFolder() kalau mau di CoreGui

    getgenv().KYS_WindowESPObjects[part] = adorn
end

function KYS_RemoveWindowAdornment(part)
    local adorn = getgenv().KYS_WindowESPObjects[part]
    if adorn then
        pcall(function() adorn:Destroy() end)
        getgenv().KYS_WindowESPObjects[part] = nil
    end
end

-- Deteksi window part dari sebuah Instance
function KYS_ResolveWindowPart(inst)
    if typeof(inst) ~= "Instance" then return nil end

    local lname = string.lower(inst.Name)

    -- Model bernama "Window" -> cari Bottom / PrimaryPart
    if lname == "window" and inst:IsA("Model") then
        local bottom = inst:FindFirstChild("Bottom", true)
    ...
    -- Part bernama "Bottom" yang parent-nya "window"
    if lname == "bottom" and inst:IsA("BasePart") then
        if inst.Parent and string.lower(inst.Parent.Name) == "window" then
            return inst
        end
    end
...
-- Handler DescendantAdded
function KYS_HandleWindowAdded(inst)
    local winPart = KYS_ResolveWindowPart(inst)
    if winPart then
        if VD.ESP_WindowEnabled and not getgenv().KYS_WindowESPObjects[winPart] then
            KYS_CreateWindowAdornment(winPart)
        end
    end
end

-- Handler DescendantRemoving
function KYS_HandleWindowRemoved(inst)
    local winPart = KYS_ResolveWindowPart(inst) or inst
    if getgenv().KYS_WindowESPObjects[winPart] then
        KYS_RemoveWindowAdornment(winPart)
    end
end

-- Scan semua window yang ada saat ini
function KYS_RefreshAllWindowESP()
    -- Bersihkan yang sudah hilang
    for part, adorn in pairs(getgenv().KYS_WindowESPObjects) do
        if not part or not part.Parent then
            KYS_RemoveWindowAdornment(part)
        end
    end

    -- Kalau mode disable, hapus semua
    if not VD.ESP_WindowEnabled then
        for part in pairs(getgenv().KYS_WindowESPObjects) do
            KYS_RemoveWindowAdornment(part)
        end
        return
    end
    ...
    for _, inst in ipairs(workspace:GetDescendants()) do
        KYS_HandleWindowAdded(inst)
    end
end

-- Setup listener (hanya sekali)
if not getgenv().KYS_WindowESP_Connected then
    getgenv().KYS_WindowESP_Connected = true

    workspace.DescendantAdded:Connect(KYS_HandleWindowAdded)
    workspace.DescendantRemoving:Connect(KYS_HandleWindowRemoved)
end
...
local ConfigFolderName = "HyunjinHub_VD"
local HttpService = game:GetService("HttpService")
...
local VD_To_Flag = {
    ...
    AutoGen_Enabled = "Bypass Generator",
...
-- CHARACTER REFS
-- =====================================================
-- Character, Humanoid, and Root are declared at the top of __HyunjinHub_Init_Main__
...
do
    ...
    local function KYS_GetESPFolder()
        ...
        local old = parent:FindFirstChild("HyunjinHub_VisualESP") or parent:FindFirstChild("ZiaanHub_ESP")
        if old then old:Destroy() end

        local folder = Instance.new("Folder")
        folder.Name = "HyunjinHub_VisualESP"
    ...
    getgenv().KYS_AddVisualESPControls = function(VisualTabRef)
        ...
        worldSection:AddDropdown({
            ...
            Values = { "Generators", "Hooks", "Gates", "Pallets", "SCP / Zombie" },
            Multi = true,
            AllowNone = true,
            Default = {},
            Callback = function(selected)
    ...
    Default = false,
    Callback = function(state)
        VD.ESP_WindowEnabled = state
        pcall(KYS_RefreshAllWindowESP)
    end,
})
...
-- =====================================================
-- BYPASS GENERATOR (NEW) - Auto Repair Semua Generator
-- =====================================================
local AutoGenData = {
    Enabled = false,
    UI = nil,
...
function VD_UpdateAutoGenButton()
    if not AutoGenData.Button then return end
...
function VD_CreateAutoGenButton()
    local parent = GetSafeGuiParent()
    if not parent then return end

    if AutoGenData.UI and AutoGenData.UI.Parent then
        AutoGenData.UI:Destroy()
    end

    local sg = Instance.new("ScreenGui")
    sg.Name = "HyunjinHub_AutoGenButton"
    sg.ResetOnSpawn = false
    sg.IgnoreGuiInset = true
    ...
    -- ===== FAB Style sama persis dengan Boost Gen Bypass =====
    ...
    -- ===== SHINE ANIMATION (sama seperti Boost Gen Bypass) =====
    local function applyShine(obj, baseColor)
        local grad = Instance.new("UIGradient", obj)
        grad.Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0, baseColor),
    ...
    applyShine(btn, Color3.fromRGB(20, 0, 30))
    applyShine(lbl, Color3.fromRGB(255, 0, 255))
    applyShine(s, Color3.fromRGB(255, 0, 255))
    -- ==========================================================
    ...
    btn.InputBegan:Connect(function(input)
        ...
        or input.UserInputType == Enum.UserInputType.Touch then
            if AutoGenData.DragLocked then return end
            AutoGenData.Dragging = true
            AutoGenData.DragStart = input.Position
            AutoGenData.DragStartPos = btn.Position
        end
    end)

    UserInputService.InputChanged:Connect(function(input)
        if not AutoGenData.Dragging or AutoGenData.DragLocked then return end
        if input.UserInputType == Enum.UserInputType.MouseMovement
        or input.UserInputType == Enum.UserInputType.Touch then
            local delta = input.Position - AutoGenData.DragStart
            btn.Position = UDim2.new(
                AutoGenData.DragStartPos.X.Scale,
                AutoGenData.DragStartPos.X.Offset + delta.X,
                AutoGenData.DragStartPos.Y.Scale,
                AutoGenData.DragStartPos.Y.Offset + delta.Y
            )
        end
    end)
    ...
    btn.InputEnded:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1
        or input.UserInputType == Enum.UserInputType.Touch then
            AutoGenData.Dragging = false
        end
    end)

    -- Klik = toggle ON/OFF (tanpa ganti warna biar konsisten shine-nya)
    btn.MouseButton1Click:Connect(function()
        AutoGenData.Enabled = not AutoGenData.Enabled
        VD_UpdateAutoGenButton()
        pcall(VD_Notify, "Bypass Generator",
            AutoGenData.Enabled and "Dengan ini kamu bisa auto escape ketika di hook" or "Nonaktif",
            2)
    end)

    AutoGenData.UI = sg
    AutoGenData.Button = btn
    VD_UpdateAutoGenButton()
end

function VD_SetAutoGenVisible(state)
    AutoGenData.Visible = state and true or false
    if state then
        VD_CreateAutoGenButton()
        if AutoGenData.Button then
            AutoGenData.Button.Visible = true
        end
    else
        AutoGenData.Enabled = false
        if AutoGenData.UI then
            pcall(function() AutoGenData.UI:Destroy() end)
            AutoGenData.UI = nil
            AutoGenData.Button = nil
        end
    end
end
getgenv().VD_SetAutoGenVisible = VD_SetAutoGenVisible

-- Loop utama: kirim RepairEvent ke semua GeneratorPoint2
task.spawn(function()
...
-- Recreate button saat respawn
LocalPlayer.CharacterAdded:Connect(function()
    task.wait(0.5)
    if AutoGenData.Visible then
        pcall(VD_CreateAutoGenButton)
        if AutoGenData.Button then AutoGenData.Button.Visible = true end
    end
end)

-- =====================================================
-- END BYPASS GENERATOR (NEW)
-- =====================================================
...
do -- FOV Tab
    ...
    KYS_AddMainInfoLine(infoPanelSection, "KystKiller", "Killer Dislay", "Off")
    ...
    visualSection:AddToggle({ Default = false, Name = "Killer Dislay", Flag = "Killer Dislay", Callback = function(v) 
        VD.VIS_KystKiller = v 
        if v then
            StartKystKiller()
        else
...
do -- Generator Tab
    ...
    Callback = function(v)
        -- Karena kamu sudah pakai KillPremium, baris "if v and false" dihapus agar fitur bisa dipakai
        if getgenv().VD_SetAutoGenVisible then
            getgenv().VD_SetAutoGenVisible(v)
        end
    end
})
    genAuto:AddDropdown({
...
pcall(function()
    VD_Notify("HyunjinHub crack", "Violence District v1.5.7 Loaded Successfully!", 5)
end)
...
function CreateRadarGUI()
    ...
    radarGui.Name = "HyunjinHub_RadarGUI"
    ...
    titleText.Text = "HyunjinHub RADAR"
...
function SetupKystKillerIndicator()
    ...
    getgenv().KYS_KystKillerGui = nil
    KYS_SetMainInfoPanelText("KystKiller", "Killer Dislay", "Waiting...")
end
        ...
        while _genv.VD and _genv.VD.VIS_KystKiller and _genv.KYS_KystKillerRunning do
            local ok, err = pcall(function()
                ...
                if #playersList == 0 then
                    KYS_SetMainInfoPanelText("KystKiller", "Killer Dislay", "No Players")
                    return
                end

                -- Sorting pakai AllowKiller + KillerChance biar akurat
                ...
                if nk then
                    local username     = (nk == LocalPlayer) and "YOU" or nk.Name
                    local killerUsed   = KYS_GetKillerUsedName(nk)
                    local killerLevel  = KYS_GetPlayerLevel(nk)

                    -- Format 4 baris: Username, KillerName, Level
                    local infoText = string.format(
                        "Username: %s\nKiller: %s\nLevel: %s",
                        username, killerUsed, killerLevel
                    )

                    KYS_SetMainInfoPanelText("KystKiller", "Killer Dislay", infoText)
                else
                    KYS_SetMainInfoPanelText("KystKiller", "Killer Dislay", "Kyst Killer: None")
                end
            end)

            if not ok then
                KYS_SetMainInfoPanelText("KystKiller", "Killer Dislay", "Error: " .. tostring(err))
            end
...
function StopKystKiller()
    ...
    KYS_SetMainInfoPanelText("KystKiller", "Killer Dislay", "Off")
end
...
do -- begin Killer Perks scope
    ...
    -- ============ HELPER FUNCTIONS (URUTAN PENTING) ============
    ...
    function KYS_GetPlayerLevel(player)
        if not player then return "?" end
        local lvl = player:GetAttribute("Level")
            or player:GetAttribute("level")
            or player:GetAttribute("LVL")
        if lvl ~= nil then return tostring(lvl) end

        local ls = player:FindFirstChild("leaderstats")
        if ls then
            local lvObj = ls:FindFirstChild("Level") or ls:FindFirstChild("LVL")
            if lvObj then return tostring(lvObj.Value) end
        end
        return "?"
    end

    function KYS_GetKillerUsedName(player)
        if not player then return "Unknown" end

        local selected = player:GetAttribute("SelectedKiller")
        if selected and type(selected) == "string" and selected ~= "" then
            return selected
        end
        ...
        if player.Character then
            ...
            if values then
                local kn = values:FindFirstChild("KillerName")
                if kn and kn:IsA("StringValue") and kn.Value ~= "" then
                    return kn.Value
                end
            end
        end

        local alt = player:GetAttribute("KillerName")
        if alt and type(alt) == "string" and alt ~= "" then
            return alt
        end
    ...
    -- ============ KYS_BuildKillerPerksText (VERSI BARU) ============
    local function KYS_BuildKillerPerksText()
        ...
        local username = killer and killer.Name or "Unknown"
        local level = killer and KYS_GetPlayerLevel(killer) or "?"

        local headerText = string.format(
            'Killer Perks [<font color="rgb(255,80,80)">%s | Lv. %s</font>]',
            KYS_EscapeRichText(username),
            KYS_EscapeRichText(level)
        )
...
function setupSpearAimbotBtn()
    ...
    print("[HyunjinHub crack] Spear Aimbot Button created successfully")
end
...
RunService.Heartbeat:Connect(function(deltaTime)
 ...
 if tick() - (getgenv()._lastWinScan or 0) > 1 then
    getgenv()._lastWinScan = tick()
    pcall(KYS_RefreshAllWindowESP)
end
...
local function KillPremium()
    ...
    print("[HyunjinHub] Premium-ограничения отключены.")
end
